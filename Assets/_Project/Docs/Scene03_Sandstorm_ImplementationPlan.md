...
### 2.3 Scene과 에셋에 아직 없는 항목

- [ ] `Scene_03_InGame_Outside`에 `SandstormZone` 루트가 없다.
- [ ] Scene에 `SandstormController`와 `SandstormZoneTrigger` 스크립트 GUID 참조가 없다.
- [ ] 실제 동선에 맞춘 Box Trigger 위치와 크기가 정해지지 않았다.
- [ ] 모래폭풍용 최종 Particle/VFX 에셋이 연결되지 않았다. Builder는 기본 Particle System 플레이스홀더만 만든다.
- [ ] 프로젝트에서 사용할 수 있는 `.wav`, `.mp3`, `.ogg`, `.aiff` 바람 SFX를 찾지 못했다.
- [ ] `windLoopSfx`가 연결되지 않았으므로 현재 코드 상태에서는 바람 소리가 재생되지 않는다.
- [ ] 모래폭풍 완료 후 이어질 다음 동선 또는 다음 콘텐츠 시작 신호가 정의되어 있지 않다.

### 2.4 명세와 다른 현재 값

- Builder의 챕터 문구는 `되살아난 모래 / 불안한 첫 발`이다.
- 명세의 정확한 문구인 `끝없는 모래 / 불안한 첫 발`로 수정해야 한다.
- 퀘스트 문구 `모래 폭풍`과 지시문 `깊은 호흡으로 모래폭풍을 잠재우세요`는 코드에 존재한다.
- 현재 UI는 프로젝트의 챕터/퀘스트 공통 프리팹을 재사용하지 않고 단순 `TextMeshPro + FaceCamera` 오브젝트로 생성된다.
- `SandstormController`는 폭풍 상승이 끝나기 전부터 `SandstormZoneTrigger`가 `MissionReady`를 활성화하므로 사용자가 연출 도중 B를 눌러 호흡을 시작할 수 있다.
- `SandstormController`는 폭풍 상승 완료 시 공용 원형 UI를 바로 표시한다. 실제 측정 시작 상태인 `BreathingActive`와 UI 표시 시점이 일치하지 않는다.
...
- [ ] 첫 번째 길 찾기 호흡과 두 번째 모래폭풍 호흡을 구분하는 미션 소유 정보를 추가한다.
  - 최소 구현: 각 콘텐츠 컨트롤러의 `isMissionOwnerActive` 가드
  - 확장 구현: `BreathMissionId` 또는 공용 Mission Context를 `PlayerStateManager`에 저장
- [ ] `SandstormController`가 다음 조건을 만족할 때만 호흡 이벤트를 처리한다.
  - Trigger 진입 완료
  - 폭풍 상승 완료
  - 현재 미션 소유자가 Sandstorm
  - 현재 플레이어 상태가 `BreathingActive`
- [ ] `Idle` 또는 다른 콘텐츠 진행 중 들어온 `OnBreathLoopCompleted`와 `OnMissionSuccess`를 무시한다.
- [ ] 폭풍 상승 완료 후에만 `SetMissionZone(true)`를 호출하도록 시작 시점을 옮긴다.
- [ ] `BreathingActive` 진입과 종료를 구독하여 지시문, 공통 원형 UI, 재도전 상태를 제어한다.
- [ ] 성공과 취소를 구분한다.
  - 성공: `Cleared → Idle`
  - 취소: 폭풍 유지 또는 최대치 복구 → `MissionReady`
- [ ] 동일 Trigger 재진입, B 연타, 중복 성공 이벤트가 상태를 두 번 바꾸지 않도록 가드를 둔다.
...
- [ ] `Scene_03_InGame_Outside`에서 Guiding Light 동선 후반에 Box Trigger를 배치한다.
- [ ] Collider의 `Is Trigger=true`, Player 감지 태그/레이어, Rigidbody 충돌 조건을 실제 XR Origin으로 검증한다.
- [ ] 챕터 UI 문구를 정확히 수정한다.
  - 제목: `끝없는 모래`
  - 부제: `불안한 첫 발`
- [ ] 퀘스트 UI에 `모래 폭풍`을 표시한다.
- [ ] 챕터/퀘스트 UI를 카메라 기준 World Space 또는 프로젝트 공통 UI 프리팹 중 하나로 통일한다.
- [ ] UI 표시 시간, 순서, Fade In/Out을 정한다.
- [ ] Trigger가 여러 Collider를 가진 XR Origin에 의해 여러 번 호출되어도 한 번만 시작되는지 확인한다.
...
- [ ] 최종 모래폭풍 Particle System 또는 VFX Prefab을 선정해 연결한다.
- [ ] 필요하면 주변에 배치된 복수 Particle System을 지원하도록 단일 필드를 배열 또는 전용 VFX Root로 변경한다.
- [ ] 진입 시 Particle System 활성화와 Emission Rate 상승을 `riseDuration` 동안 보간한다.
- [ ] 기본 Fog Density `0.01`과 최대 Fog Density를 Quest 가독성 기준으로 튜닝한다.
  - 현재 코드 기본 최대값: `0.12`
- [ ] 호흡 루프 완료마다 목표 강도를 `1 - completedLoops / targetLoopCount`로 계산한다.
- [ ] 연속 이벤트가 들어올 때 이전 Lerp를 중단하고 현재값에서 다음 목표값으로 자연스럽게 이어지게 한다.
- [ ] 세 번째 루프에서 성공 이벤트와 루프 이벤트가 같은 프레임에 연달아 와도 최종값이 0과 원래 Fog 값으로 확정되게 한다.
- [ ] 파티클이 카메라 바로 앞을 과도하게 가리거나 Quest GPU Overdraw를 유발하지 않도록 튜닝한다.
...
- [ ] 루프 가능한 바람 SFX 에셋을 추가하고 Import Settings를 확인한다.
- [ ] `windLoopSfx`에 Clip을 연결한다.
- [ ] 진입 시 Fade In, 호흡 루프 완료 시 폭풍 강도에 비례한 Volume 감소, 성공 시 Fade Out을 구현한다.
- [ ] `SoomAudioManager`의 SFX 볼륨을 반영한다.
- [ ] Scene 전환, 취소, 비활성화 시 AudioSource가 반드시 정지되는지 확인한다.
- [ ] 2D 앰비언트와 3D 공간음 중 실제 플레이 감각에 맞는 방식을 확정한다.
...
- [ ] 첫 번째 호흡 성공으로 생성된 활성 Guiding Light 인스턴스를 Sandstorm에 전달한다.
- [ ] Trigger 진입 시 해당 인스턴스의 `GuidingLightController`를 검증한다.
- [ ] 폭풍 상승과 함께 실제 `Light.intensity`를 `lightFadeDuration` 동안 0으로 보간한다.
- [ ] 필요하면 이동도 멈출지 결정한다.
  - 기능 명세는 빛 강도만 요구하므로 기본 계획은 이동 상태를 변경하지 않는다.
- [ ] 폭풍 성공 시 Awake에서 저장한 원래 강도로 복구한다.
- [ ] 등불이 파괴되었거나 참조가 없을 때 Null 예외 없이 폭풍을 진행한다.
...
- [ ] 공용 `BreathCircleUI`는 `BreathingActive` 진입 시 한 번만 `Show()`한다.
- [ ] 모래폭풍 지시문을 확정한다.
  - 권장: `깊게 호흡하여 모래폭풍을 잠재우세요`
- [ ] 콘텐츠 B 전용 `BreathMissionGuideController`가 Sandstorm 세션에는 반응하지 않게 한다.
- [ ] 루프 시작 시 슬롯 3개가 비어 있고, 매 루프당 정확히 하나씩 채워지는지 확인한다.
- [ ] 취소 후 재시작 시 슬롯, 구슬 위치, 스케일, 호흡값을 초기화한다.
- [ ] 세 번째 구슬 이동 Animation과 성공 Pulse가 UI Hide보다 먼저 보이도록 이벤트 순서 또는 지연 시간을 조정한다.
- [ ] 폭풍 감소 피드백과 구슬 채움 타이밍을 같은 루프 완료 이벤트에 맞춘다.
...
- [ ] 메뉴 이름을 역할이 명확하게 드러나도록 변경한다.
  - 권장: `SOOM/Scene 03/Setup Sandstorm Breath Zone`
- [ ] 기존 `SandstormZone`이 있으면 즉시 종료하지 말고 자식과 컴포넌트를 찾아 생성 또는 갱신한다.
- [ ] 반복 실행해도 Trigger, Controller, UI, Particle, AudioSource가 중복되지 않게 한다.
- [ ] 챕터 문구를 `끝없는 모래 / 불안한 첫 발`로 수정한다.
- [ ] `BreathEventsChannel.asset`과 Scene의 기존 `BreathCircle_Outside`를 연결한다.
- [ ] 런타임 Guiding Light 획득 방식에 맞는 소유자 또는 이벤트 참조를 연결한다.
- [ ] 실제 VFX Prefab과 Wind Clip 경로가 확정되면 자동 연결한다.
- [ ] 기존 플레이스홀더 Particle/UI를 최종 에셋으로 마이그레이션하되 사용자가 조정한 Transform은 보존한다.
- [ ] 필수 참조가 없으면 빌드를 완료한 것처럼 로그하지 말고 항목별 경고를 남긴다.
- [ ] 실행 후 Scene을 Dirty 처리하고 사용자가 저장할 수 있게 한다.
...
- [ ] 모래폭풍 성공 직후 다음 진행 조건을 정의한다.
  - Guiding Light 이동 재개
  - 다음 Waypoint 활성화
  - 다음 Trigger 또는 Ending Sequence 활성화
- [ ] `Cleared`가 저장되어 Trigger 재진입으로 폭풍이 다시 시작되지 않게 한다.
- [ ] 성공 후 `PlayerStateManager`가 `Idle` 또는 다음 콘텐츠가 요구하는 상태로 정확히 전환되는지 확인한다.
- [ ] Scene 재시작 정책을 정한다.
...
- [ ] 첫 번째 호흡 성공이 SandstormController에 잘못 반영되지 않는다.
- [ ] Guiding Light를 따라 Box Trigger에 진입하면 폭풍이 한 번 시작된다.
- [ ] 챕터/퀘스트 문구가 명세와 일치한다.
- [ ] 모래 Particle, Fog, Wind SFX가 점진적으로 강해진다.
- [ ] 실제 Guiding Light 인스턴스의 빛이 0으로 감소한다.
- [ ] 폭풍 상승 완료 후 B로 두 번째 호흡을 시작한다.
- [ ] 공통 원형 UI가 3회 판정을 표시한다.
- [ ] 콘텐츠 B 안내문과 모래폭풍 안내문이 충돌하지 않는다.
- [ ] 호흡 1회마다 Particle과 Fog가 단계적으로 감소한다.
- [ ] 3회 성공 후 폭풍, Fog, SFX가 완전히 정리된다.
- [ ] Guiding Light의 빛이 원래 강도로 복구된다.
- [ ] 취소 후 재도전할 수 있다.
- [ ] 성공 후 다음 콘텐츠로 진행할 수 있다.
- [ ] Builder 반복 실행 시 Hierarchy와 UI가 중복되지 않는다.
